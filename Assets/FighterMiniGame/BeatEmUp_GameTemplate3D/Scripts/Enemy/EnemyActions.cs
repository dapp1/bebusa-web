...
public class EnemyActions : MonoBehaviour
{
  ...
  [SerializeField] private float coinFallSpeed = 1f; //coin fall speed
  ...
  public int moneyValue = 1; //the amount of money this enemy is worth
  ...
  public void DestroyUnit()
  {
    ...
    FighterMoney.deathEnemyCount++;
    Debug.Log("deathEnemyCount:" + FighterMoney.deathEnemyCount);

    StartCoroutine(SpawnCoins());
    IEnumerator SpawnCoins()
    {

      if (Random.Range(0, 100) < FighterMoney.GetChance())
      {
        for (int i = 0; i < FighterMoney.GetCoinCount(); i++)
        {
          SpawnCoin();
  ...
  private void SpawnCoin()
  {
    ...
    coin.transform.position += Vector3.up * 2f + Vector3.left * Random.Range(-0.3f, 0.3f);

    Vector3 velocity = Vector3.up * 3f;

    Debug.Log("velocity:" + velocity);
    coin.GetComponent<Rigidbody>().velocity = velocity;
  }
